...
export function ChatMessage({ message, username, isLast }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* max-w capped at 82% on desktop, full-ish on mobile */}
      <div className={cn("flex flex-col gap-1 max-w-[88%] sm:max-w-[82%]", isUser ? "items-end" : "items-start")}>
        <div
          className={cn(
            "px-4 py-3 text-sm leading-relaxed",
            isUser ? "rounded-2xl rounded-tr-sm" : "rounded-2xl rounded-tl-sm chat-prose"
          )}
          style={
            isUser
        ...
        >
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded transition-colors hover:text-[var(--text-secondary)]"
...
function Avatar({ isUser, username }: { isUser: boolean; username?: string }) {
  if (isUser) {
    return (
      <div
        className="w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-semibold flex-shrink-0 mt-1"
